...
class Test(EthTesterCase):
    ...
    def test_hash(self):
        nonce_oracle = RPCNonceOracle(self.accounts[0], self.rpc)
        c = BB(self.chain_spec, signer=self.signer, nonce_oracle=nonce_oracle)
        (tx_hash, o) = c.add(self.address, self.accounts[0], hash_of_bar, topic=hash_of_foo)
        self.conn.do(o)
        o = receipt(tx_hash)
        r = self.conn.do(o)
        self.assertEqual(r['status'], 1)

        (tx_hash, o) = c.add(self.address, self.accounts[0], hash_of_baz, topic=hash_of_foo)
        self.conn.do(o)
        o = receipt(tx_hash)
        r = self.conn.do(o)
        self.assertEqual(r['status'], 1)

        (tx_hash, o) = c.add(self.address, self.accounts[0], hash_of_foo, topic=hash_of_bar)
        self.conn.do(o)
        o = receipt(tx_hash)
        r = self.conn.do(o)
        self.assertEqual(r['status'], 1)
        ...
        o = c.entry(self.address, self.accounts[0], 1, topic=hash_of_foo, sender_address=self.accounts[0])
        r = self.conn.do(o)
